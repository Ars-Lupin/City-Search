...
void heapPush(Heap *heap, dataType data) {
    ...
    while (idx > 0) {
        ...
        if (heap->cmpFn(data, vectorGet(heap->v, idxFather)) < 0) {
...
void *heapPop(Heap *heap) {
    ...
    while(1) {
        ...
        if ((leftChild < vectorSize(heap->v)) &&
            (heap->cmpFn(vectorGet(heap->v, leftChild), vectorGet(heap->v, min))) < 0) {
                min = leftChild;
            }
        
        if ((rightChild < vectorSize(heap->v)) &&
            (heap->cmpFn(vectorGet(heap->v, rightChild), vectorGet(heap->v, min))) < 0) {
                min = rightChild;
            }
